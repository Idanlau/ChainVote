...
class voterVerifierClass1:
    ...
    def processVoter(self, voterInfo):
        ...
        dobAdder = int(str(voterInfo['Date of Birth'])[0:4])
        dobAdder += int(self.getDobNum(str(voterInfo['Date of Birth'])[5:6] , self.dobM) * self.dobMModifier)
        dobAdder += int(self.getDobNum(str(voterInfo['Date of Birth'])[8:9], self.dobD) * self.dobDModifier)
        dobAdder += int(self.dobOverallModifier)
